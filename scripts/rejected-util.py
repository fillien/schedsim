...
def main():
    ...
    results.to_csv("data-rejected.csv", index=False, sep=" ")
    subprocess.run(["gnuplot", "./scripts/plot-rejected.gp"])
...
def run_scenario(schedview, log_path):
    energy = subprocess.run(
        [schedview, log_path, "--cli", "--rejected"], capture_output=True, text=True
    ).stdout.strip()
    return float(energy)
