...
tracer::~tracer() {
	barectf_platform_simulator_fini(platform_ctx);
}
...
void tracer::add_trace(const trace& new_trace) {
	...
	case SERV_BUDGET_EXHAUSTED: barectf_trace_serv_budget_exhausted(ctx, new_trace.target_id); break;
	case SERV_BUDGET_REPLENISHED: barectf_trace_serv_budget_replenished(ctx, new_trace.target_id, new_trace.payload); break;
	...
	case VIRTUAL_TIME_UPDATE: barectf_trace_virtual_time(ctx, new_trace.target_id, new_trace.payload); break;
	default: break;
	}
