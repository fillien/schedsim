...
protected:
    void SetUp() override {
        ...
        pd_ = &engine_.platform().add_power_domain({
            ...
            {1, CStateScope::PerProcessor, duration_from_seconds(0.01), Power{10.0}}
        });
...
TEST_F(EnergyTrackerTest, MultiProcessorEnergy) {
    ...
    // Run for 1 second
    engine2.run(time_from_seconds(1.0));
...
TEST_F(EnergyTrackerTest, EnergyWithDVFSDelay) {
    ...
    auto& cd = engine2.platform().add_clock_domain(
        Frequency{500.0}, Frequency{2000.0}, duration_from_seconds(0.1));
    auto& pd = engine2.platform().add_power_domain({
        {0, CStateScope::PerProcessor, duration_from_seconds(0.0), Power{100.0}}
    });
    ...
    engine2.run(time_from_seconds(0.5));
    ...
    engine2.run(time_from_seconds(0.65));
