...
ffa_delay_timer::ffa_delay_timer(const std::weak_ptr<engine> sim) : sched_parallel(sim)
{
...
auto ffa_delay_timer::compute_freq_min(
...
auto ffa_delay_timer::get_nb_active_procs(const double& new_utilization = 0) const -> std::size_t
{
...
void ffa_delay_timer::change_state_proc(
...
auto ffa_delay_timer::cores_on_sleep() -> std::size_t
{
        using enum processor::state;
...
void ffa_delay_timer::activate_next_core()
{
        using enum processor::state;
...
void ffa_delay_timer::put_next_core_to_bed()
{
        using enum processor::state;
        auto& processors = sim()->chip()->processors;
        auto it = std::find_if(processors.begin(), processors.end(), [](const auto& proc) {
...
void ffa_delay_timer::adjust_active_processors(std::size_t target_processors)
{
...
void ffa_delay_timer::update_platform()
{
