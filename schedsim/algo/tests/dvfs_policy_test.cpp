...
TEST_F(DvfsPolicyTest, CooldownTimer_CanChange_Initially) {
    CooldownTimer timer(engine_, duration_from_seconds(1.0));
...
TEST_F(DvfsPolicyTest, PowerAware_CooldownPreventsChange) {
    ...
    // 1 second cooldown
    PowerAwareDvfsPolicy policy(engine_, duration_from_seconds(1.0));
...
TEST_F(DvfsPolicyTest, PowerAware_CooldownPeriod) {
    PowerAwareDvfsPolicy policy(engine_, duration_from_seconds(2.5));
    EXPECT_DOUBLE_EQ(duration_to_seconds(policy.cooldown_period()), 2.5);
}
