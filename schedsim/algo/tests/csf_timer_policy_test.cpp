...
class CsfTimerPolicyTest : public ::testing::Test {
protected:
    void SetUp() override {
...
TEST_F(CsfTimerPolicyTest, ZeroCooldown_ImmediateApplication) {
    ...
    CsfTimerPolicy policy(engine_, duration_from_seconds(0.0));

    policy.on_utilization_changed(sched, *cd_);

    // With zero utilization and freq_eff=1000, CSF should apply freq_eff
    EXPECT_DOUBLE_EQ(cd_->frequency().mhz, 1000.0);
}

TEST_F(CsfTimerPolicyTest, DeferredApplication_TimerFires) {
    engine_.platform().finalize();
    std::vector<Processor*> proc_vec(procs_, procs_ + 4);
    EdfScheduler sched(engine_, proc_vec);
    sched.enable_grub();

    CsfTimerPolicy policy(engine_, duration_from_seconds(1.0));

    EXPECT_DOUBLE_EQ(cd_->frequency().mhz, 2000.0);

    policy.on_utilization_changed(sched, *cd_);

    // Frequency should NOT have changed yet
    EXPECT_DOUBLE_EQ(cd_->frequency().mhz, 2000.0);
...
TEST_F(CsfTimerPolicyTest, TimerReset_OnNewUtilChange) {
    ...
    CsfTimerPolicy policy(engine_, duration_from_seconds(2.0));
    ...
    // New utilization change: cancel old timer, schedule new at t=3.0
    sched.add_server(task, duration_from_seconds(1.0), duration_from_seconds(10.0));
    policy.on_utilization_changed(sched, *cd_);

    // Advance past original timer time but before new timer
...
TEST_F(CsfTimerPolicyTest, NoChangeNeeded_NoTimer) {
    engine_.platform().finalize();
    std::vector<Processor*> proc_vec(procs_, procs_ + 4);
    EdfScheduler sched(engine_, proc_vec);
    sched.enable_grub();

    // Apply immediate CSF first
    CsfPolicy immediate_policy(engine_);
    immediate_policy.on_utilization_changed(sched, *cd_);
    Frequency settled_freq = cd_->frequency();

    // Timer policy: target matches => no timer
    CsfTimerPolicy timer_policy(engine_, duration_from_seconds(1.0));
    timer_policy.on_utilization_changed(sched, *cd_);

    EXPECT_DOUBLE_EQ(cd_->frequency().mhz, settled_freq.mhz);
}

TEST_F(CsfTimerPolicyTest, EnableCsfTimerConvenience) {
    engine_.platform().finalize();
    std::vector<Processor*> proc_vec(procs_, procs_ + 4);
    EdfScheduler sched(engine_, proc_vec);

    sched.enable_csf_timer(duration_from_seconds(0.5), 1);

    SUCCEED();
}

TEST_F(CsfTimerPolicyTest, FrequencyCallbackInvoked) {
    engine_.platform().finalize();
    std::vector<Processor*> proc_vec(procs_, procs_ + 4);
    EdfScheduler sched(engine_, proc_vec);
    sched.enable_grub();
