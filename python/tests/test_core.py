...
def test_module_exports():
    ...
    # Enums
    assert hasattr(sim, "ProcessorState"), "ProcessorState enum not exported"
    assert hasattr(sim, "CStateScope"), "CStateScope enum not exported"
...
def test_platform_building():
    ...
    assert proc.state == sim.ProcessorState.Idle, "Processor should be Idle initially"
...
def test_simple_run():
    ...
    assert abs(engine.time - 10.0) < 1e-9, f"Expected time~=10.0, got {engine.time}"
...
def test_enum_values_exist():
    ...
    assert sim.ProcessorState.Idle is not None
    assert sim.ProcessorState.ContextSwitching is not None
    assert sim.ProcessorState.Running is not None
    assert sim.ProcessorState.Sleep is not None
    assert sim.ProcessorState.Changing is not None

    # CStateScope enum values
    assert sim.CStateScope.PerProcessor is not None
    assert sim.CStateScope.DomainWide is not None

    # DeadlineMissPolicy enum values
    assert sim.DeadlineMissPolicy.Continue is not None
    assert sim.DeadlineMissPolicy.AbortJob is not None
    assert sim.DeadlineMissPolicy.AbortTask is not None
    assert sim.DeadlineMissPolicy.StopSimulation is not None
